...
def test(n, method):
    ...
    average_turns = float(score)/float(n)
    print "Average turns = ", float(score)/float(n)
    return (score, average_turns)
...
# Default is to solve MDP and play 1 game
    ...
    plt.title("Discount Factor vs. Average Turns Per Game")
...
def main():
    throw.init_board()
    num_games = 100
    modelfree.Q_learning(GAMMA, num_games, 0.3)
#************************************************#
# Uncomment the lines below to run the mdp code, #
# using the simple dart thrower that matches     #
# the thrower specified in question 2.           #
...
# the modelbased algorithm.
    #random.seed()
    #throw.init_thrower()
    #modelbased.modelbased(GAMMA, 3, 10)
"""
    plt.clf()
    xs = np.linspace(5,40,8)
    ys = []
    for epoch in xs:
      random.seed()
      throw.init_thrower()
      ys.append(modelbased.modelbased(GAMMA, epoch, 100))
    print xs
    print ys
    
    pl = plt.plot(xs,ys,color='b')

    plt.title("Epoch Size vs. Average Turns Per Game")
